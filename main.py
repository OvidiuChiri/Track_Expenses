...
def listAllExpenses():
    ...
    # Executing the SQL SELECT command to retrive the data from the database table.
...
def removeExpense():
    ...
    confirmation = mb.askyesno('Are you sure?', f'Are you sure you want to delete the record of {valuesSelected[2]}')
...
def removeAllExpenses():
    ...
    # displaying a message box asking for confirmation.
    confirmation = mb.askyesno('Are you sure?', 'Are you sure that you want to delete all the expense items from the '
                                                'database?', icon='warning')

    # if the user say YES, deleting the entries from the table and executing the SQL DELETE FROM command to delete
    # all the entries
    if confirmation:
        data_table.delete(*data_table.get_children())
...
def addAnotherExpense():
    ...
    if not dateField.get() or not payee.get() or not description.get() or not modeOfPayment.get():
...
def editExistingExpense(editSelectedButton=None):
    ...
    dbconnector.execute(
        'UPDATE ExpenseTracker SET Date = ?, Payee = ?, Description = ?, Amount = ?, ModOfPayment = ? WHERE ID = ?',
...
def expenseToWordsBeforeAdding():
    ...
    if not dateField.get() or not payee.get() or not description.get() or not amount.get() or not modeOfPayment.get():
        ...
        # defining the message to be displayed in the message box
        msg = f'Your expense can be read like: \n' \
              f'"You paid {amount.get()} to {payee.get()} for {description.get()} on {dateField.get_date()} via {modeOfPayment.get()}"'
...
if __name__ == "__main__":
    ...
    dbconnector.execute(
        'CREATE TABLE IF NOT EXISTS ExpenseTracker(ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, Date DATETIME, Payee TEXT'
        'Description TEXT, Amount FLOAT,ModeOfPayment TEXT)'
    )
    ...
    main_win.title('Expense Tracker - Ovidiu')
    ...
    convertSelectedButton.grid(row=2, column=0, sticky=W, padx=10, pady=10)
    deleteButton.grid(row=1, column=0, sticky=W, padx=10, pady=10)
    deleteAllButton.grid(row=1, column=0, sticky=W, padx=10, pady=10)
    ...
    Xaxis_Scrollbar.pack(side=RIGHT, fill=Y)
    ...
    data_table.heading('ModeOfPayment', text='ModeOfPayment', anchor=CENTER)

    # Adding different colums to the table.
